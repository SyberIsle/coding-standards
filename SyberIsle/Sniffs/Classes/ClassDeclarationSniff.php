 ...
 *
 * Modified to put extends/implements on a new line because it's more readable than PSR-2
 *
 * PHP version 5
 *
 * @category  PHP
...
namespace SyberIsle\Sniffs\Classes;

use PHP_CodeSniffer\Files\File;
use PHP_CodeSniffer\Standards\PEAR\Sniffs\Classes\ClassDeclarationSniff as PearClassDeclarationSniff;
use PHP_CodeSniffer\Util\Tokens;

if (class_exists(PearClassDeclarationSniff::class, true) === false) {
	$error = 'Class ' . PearClassDeclarationSniff::class . ' not found';
...
class ClassDeclarationSniff
	extends PearClassDeclarationSniff
{
	/**
		...
		foreach (array('extends', 'implements') as $keywordType) {
			...
			if ($keyword !== false) {
				if ($tokens[$keyword]['line'] === $tokens[$stackPtr]['line']) {
					$error = 'The ' . $keywordType . ' keyword must be below the %s name';
					...
					// in the list.
					$token = $tokens[($keyword - 1)]['content'];
					$gap   = strlen($token);
					if ($token === "\n") {
						$error = 'Expected 1 tab before ' . $keywordType . ' keyword; none found';
						$data  = array($gap);
						$fix   = $phpcsFile->addFixableError($error, $keyword, 'TabBefore' . ucfirst($keywordType), $data);
						if ($fix === true) {
							$phpcsFile->fixer->replaceToken(($keyword - 1), "\t");
						}
					}
					elseif ($gap !== 4) {
						$error = 'Expected 1 tab before ' . $keywordType . ' keyword; too much whitespace found';
						$fix   = $phpcsFile->addFixableError($error, $keyword, 'WhiteSpaceBefore' . ucfirst($keywordType));
						if ($fix === true) {
							$phpcsFile->fixer->replaceToken(($keyword - 1), "\t");
						}
					}
